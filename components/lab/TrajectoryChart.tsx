...
export default function TrajectoryChart({a,b,color,compare}:{a:SimFrame[];b:SimFrame[];color:string;compare:boolean}){
 ...
 return <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm"><div className="text-[12pt] font-bold text-gray-900">FINGER TRAJECTORY</div><div className="text-[10pt] text-gray-500">Simulated 10 seconds of movement</div>
  <div className="flex gap-3 mt-3">{NM.map((n,k)=><button key={n} onClick={()=>setTab(k)} className={`text-[10pt] pb-1 border-b-2 ${tab===k?'text-[#0F6E5E] border-[#0F6E5E]':'text-gray-400 border-transparent'}`}>{n}</button>)}</div>
  <svg viewBox="0 0 600 140" className="w-full mt-3 bg-[#fafafa] rounded-lg">{[0,30,60,90].map(v=><g key={v}><line x1="30" x2="590" y1={Y(v)} y2={Y(v)} stroke="#f3f4f6"/><text x="24" y={Y(v)+3} textAnchor="end" fontSize="9" fill="#9ca3af">{v}°</text></g>)}
