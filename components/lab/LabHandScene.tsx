...
export default function LabHandScene({label,color,src,src2,compare,running}:{label:string;color:string;src:Src;src2:Src;compare:boolean;running:boolean}){
 return <div className="bg-[#0f1923] rounded-2xl overflow-hidden h-[340px] flex flex-col">
  <div className="h-8 bg-[#0a0a0a]/60 flex items-center px-4 gap-3 shrink-0 font-mono text-[9pt] text-[#f0f0f0]"><span className="w-2 h-2 rounded-full" style={{background:color}}/>{label}<span className="ml-auto text-[8pt] text-[#0F6E5E]">{running?'SIM':'READY'}</span></div>
