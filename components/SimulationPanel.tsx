...
import {EXERCISES} from '@/lib/simExercises'
...
export default function SimulationPanel(){
 ...
 <motion.div initial={false} animate={{height:open?240:0,opacity:open?1:0}} transition={{duration:.3,ease:'easeInOut'}} className="fixed bottom-0 inset-x-0 z-50 bg-[#0d0d0d] border-t border-[#1f1f1f] overflow-y-auto" style={{pointerEvents:open?'auto':'none'}}>
  ...
  <div className="flex items-center gap-2 mt-3 flex-wrap"><span className="text-[8pt] font-mono text-[#333]">PRESETS:</span>
   {Object.entries(EXERCISES).map(([k,x])=><button key={k} className={btn(sim.preset===k)} onClick={()=>setSim({f:x.f,emg:x.emg,pitch:x.pitch,roll:x.roll,yaw:x.yaw,preset:k,auto:false})}>{x.label}</button>)}
