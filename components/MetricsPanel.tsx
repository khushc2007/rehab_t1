...
const pad={padding:'clamp(8px,1.4vh,14px)'}
...
export default function MetricsPanel({mode,sel,onSelect,onStart,onStop,tremor}:{mode:Mode;sel:Ex;onSelect:(e:Ex)=>void;onStart:()=>void;onStop:()=>void;tremor:boolean}){
 ...
 return <aside className="w-[30%] h-full bg-[#111] border-l border-[#1f1f1f] flex flex-col font-mono overflow-y-auto" style={{padding:'clamp(12px,2.2vh,24px)',gap:'clamp(4px,1vh,14px)'}}>
  ...
  <div className={card} style={pad}>
   ...
   <div className="leading-none my-1"><span className="text-[30pt]" style={{color:idle?'#333':'#f0f0f0'}}>{idle?'——':String(s.repCount).padStart(2,'0')}</span><span className="text-[18pt] text-[#333]"> / {idle?'——':s.targetReps}</span></div>
  ...
  <div className={card} style={pad}><div className={`${lab} mb-2`}>FINGERS</div>
   {N.map((n,i)=>{const v=Math.round(s.fingers[i]),dot=v>=65&&v<=85?'#2ea853':v>=50?'#f5a623':'#555'
    return <div key={n} className={`flex items-center h-5 ${i===4?'border-t border-[#1a1a1a]':''}`}><span className="w-1.5 h-1.5 rounded-full mr-2" style={{background:dot}}/>
